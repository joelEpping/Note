...
public class VendaDAO implements IDAO<Venda> {
    ...
    @Override
    public void insertar(Venda venda) throws Exception {
        ...
        int idVenda = rs.getInt(1);
         c.confirmar();
        JOptionPane.showMessageDialog(null, "codigo de venta"+idVenda);
    ...
    @Override
    public void eliminar(Venda venda) throws Exception {
        Conexion c = new Conexion();
        String sql = "UPDATE TBVENDA SET CODIGOCLIENTE=?, DATAVENDA=?, VALORTOTAL=?, SITUACAO=? WHERE CODIGO=?";
        PreparedStatement ps = c.getConexao().prepareStatement(sql);
        ps.setInt(1, venda.getCliente().getCodigo());
